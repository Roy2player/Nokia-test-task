...
void readFiles(mem_block_t* block, mem_file_img_t* file) {
    ...
    mem_block_t* curr_block = block; // Текущий блок памяти
    uint8_t* curr_block_buf = curr_block->buf; // Заполняемый бит буфера
    size_t curr_block_size = curr_block->mb_size; // Сколько места в буфере
    ...
    while (true) { // Цикл заполнения блоков памяти
        ...
        while (true) {
            ...
            if (curr_block_size == 0) { // Если блок памяти заполнен
                curr_block = curr_block->next_mb; // Перемещаемся к следующему блоку

                if (!curr_block) { // Если блоки памяти закончились
                    return; // Выходим из функции
                }

                curr_block_buf = curr_block->buf; // Обновляем указатель на текущий заполняемый бит
                curr_block_size = curr_block->mb_size; // Обновляем размер буфера
            }
        }
    }
}
...
int main()
{
    ...
    mem_block_t* mem_block_t = NULL; // Объявляем блок памяти
    while (numbers_block--) { // Заполняем блок(и) памяти
        mem_block_t = add_mem_block_t(mem_block_t, size_block);
    }

    cout << "\nБлоки памяти до записи:" << endl;
    print_mem_block_t(mem_block_t);

    WIN32_FIND_DATAA file_struct; // Структура файла
    mem_file_img_t* mem_file_img_t = NULL; // Объявляем список файлов
    ...
    if (h_file_struct != INVALID_HANDLE_VALUE) { // Если файл найден
        do {
            if((file_struct.dwFileAttributes & FILE_ATTRIBUTE_DIRECTORY) == 0){ // Если это не дирректория
                mem_file_img_t = add_file_img_t(mem_file_img_t, (char*)file_struct.cFileName); // Заполняем список файлов
    ...
    readFiles(mem_block_t, mem_file_img_t); // Заполняем блоки памяти
